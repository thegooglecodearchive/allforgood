...
def form_base_query(args):
  ...
  if api.PARAM_VOL_STARTDATE in args or api.PARAM_VOL_ENDDATE in args:
    ...
    if api.PARAM_VOL_STARTDATE in args and args[api.PARAM_VOL_STARTDATE] != "":
      try:
        ...
        logging.error('base_search.form_base_query malformed start date: %s' % 
    ...
    if api.PARAM_VOL_ENDDATE in args and args[api.PARAM_VOL_ENDDATE] != "":
      try:
        ...
        logging.error('base_search.form_base_query malformed end date: %s' % 
  ...
  if api.PARAM_VOL_PROVIDER in args and args[api.PARAM_VOL_PROVIDER] != "":
    if re.match(r'[a-zA-Z0-9:/_. -]+', args[api.PARAM_VOL_PROVIDER]):
      ...
      # TODO: throw 500
      logging.error('base_search.form_base_query illegal providername: ' + 
                    args[api.PARAM_VOL_PROVIDER])
...
def query(query_url, args, cache):
  ...
  if fetch_result.status_code != 200:
    logging.error('base_search.query Base fetch returned status=%s url=%s' %
                  (str(fetch_result.status_code), query_url))
    return result_set
  ...
  for i, entry in enumerate(elems):
    ...
    if not url:
      logging.warning('base_search.query skipping missing detailurl in %d' % i)
      continue
    ...
    if res.event_date_range:
      ...
      if not match:
        logging.warning('base_search.query skipping %d, bad date range %s-%s' %
...
def get_from_ids(ids):
  ...
  if datastore_missing_ids:
    logging.warning('base_search.get_from_ids no entry for %s' %
                    datastore_missing_ids)
  ...
  for (item_id, volunteer_opportunity_entity) in datastore_results.iteritems():
    if not volunteer_opportunity_entity.base_url:
      logging.warning('base_search.get_from_ids no base_url for %s' % item_id)
      continue
    ...
    if temp_results.results[0].item_id != item_id:
      logging.error('base_search.get_from_ids first result not expected result ' + 
                    'Expected: %s Found: %s len(results): %d' %
