...
def make_ascii_string(str):

  try:
    rtn = str.decode('utf-8', 'replace')
  except:
    rtn = str.decode('utf-8', 'ignore')
    
  try:
   rtn = rtn.encode('ascii', 'asciify')
  except:
    # encode as ascii and drop errors, same as before
    rtn = rtn.encode('ascii', 'ignore')
...
def main():
  ...
  if not sys.argv[1:]:
    for line in sys.stdin:
      print make_ascii_string(line),
    print
  else:
    for arg in sys.argv[1:]:
      make_ascii_string(arg)
